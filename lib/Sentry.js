...
export const Sentry = {
    ...
    if (
      ...
      Sentry.eventEmitter.addListener(
        ...
        event => {
          ...
          if (Sentry._eventSentSuccessfully)
            Sentry._eventSentSuccessfully(event);
        }
      );
  ...
  setShouldSendCallback(callback) {
    ...
    if (Sentry._ravenClient)
      Sentry._ravenClient.setShouldSendCallback(callback);
  },
  ...
  setUserContext(user) {
    ...
    if (Sentry.isNativeClientAvailable())
      Sentry._nativeClient.setUserContext(user);
  },
  ...
  setTagsContext(tags) {
    ...
    if (Sentry.isNativeClientAvailable())
      Sentry._nativeClient.setTagsContext(tags);
  },
  ...
  setExtraContext(extra) {
    ...
    if (Sentry.isNativeClientAvailable())
      Sentry._nativeClient.setExtraContext(extra);
  },
  ...
  _breadcrumbCallback(crumb) {
    if (Sentry.isNativeClientAvailable())
      Sentry._nativeClient.captureBreadcrumb(crumb);
  },

  _captureEvent(event) {
    if (Sentry.isNativeClientAvailable())
      Sentry._nativeClient.captureEvent(event);
  },
